...
import { recordDonationOnChain } from "../../blockchain/recordDonation";
...
const Booked = ({ requests }) => {
  ...
  const handleMoveToStatus = async (id, status) => {
    try {
      setLoading(true);
      if (status === "closed") {
        const requestToLog = requests.find((req) => req._id === id);
        console.log("Request to be closed:", requestToLog);
        const success = await recordDonationOnChain(requestToLog);
        if (!success) {
          setLoading(false);
